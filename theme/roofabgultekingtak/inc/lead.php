 ...
 * or photo. They call ro_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Roofab Gulteking Tak AB.
 *
 ...
 * @return string
 */
function ro_sample_note() {
	return ro_lead( 'ar_riktig' ) ? '' : ' <span class="ro-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ro_lead_img().
 */
function ro_lead_bild( $i ) {
	$bilder = (array) ro_lead( 'bilder', array() );
 ...
 * @return string
 */
function ro_ring_label() {
	$namn = ro_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ro_lead( 'telefon' ) ) : 'Ring ' . ro_lead( 'telefon' );
}
