...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'roofabgultekingtak-leads' ),
					'singular_name' => __( 'Lead', 'roofabgultekingtak-leads' ),
					'edit_item'     => __( 'Förfrågan', 'roofabgultekingtak-leads' ),
					'search_items'  => __( 'Search leads', 'roofabgultekingtak-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'roofabgultekingtak-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'roofabgultekingtak-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'roofabgultekingtak-leads' ),
			'npl_phone'    => __( 'Phone', 'roofabgultekingtak-leads' ),
			'npl_service'  => __( 'Needs', 'roofabgultekingtak-leads' ),
			'npl_timeline' => __( 'When', 'roofabgultekingtak-leads' ),
			'npl_zip'      => __( 'ZIP', 'roofabgultekingtak-leads' ),
			'npl_source'   => __( 'Source', 'roofabgultekingtak-leads' ),
			'npl_status'   => __( 'Status', 'roofabgultekingtak-leads' ),
			'date'         => __( 'Received', 'roofabgultekingtak-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'roofabgultekingtak-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'roofabgultekingtak-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'roofabgultekingtak-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'roofabgultekingtak-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'roofabgultekingtak-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'roofabgultekingtak-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'roofabgultekingtak-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'roofabgultekingtak-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'roofabgultekingtak-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'roofabgultekingtak-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'roofabgultekingtak-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'roofabgultekingtak-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
